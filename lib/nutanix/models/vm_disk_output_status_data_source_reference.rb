...
module Nutanix
  class VmDiskOutputStatusDataSourceReference
    ...
    attr_accessor :is_direct_attach
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::VmDiskOutputStatusDataSourceReference` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::VmDiskOutputStatusDataSourceReference`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'is_direct_attach')
        self.is_direct_attach = attributes[:'is_direct_attach']
      end
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^((http[s]?):\/)?\/?([^:\/\s]+)((\/\w+)*(:[0-9]+)*?\/)([\w\-\.]+[^#?\s]+)(.*)?(#[\w\-]+)?$/)
    ...
    def valid?
      ...
      return false if !@url.nil? && @url !~ Regexp.new(/^((http[s]?):\/)?\/?([^:\/\s]+)((\/\w+)*(:[0-9]+)*?\/)([\w\-\.]+[^#?\s]+)(.*)?(#[\w\-]+)?$/)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          is_direct_attach == o.is_direct_attach &&
          url == o.url &&
          kind == o.kind &&
          uuid == o.uuid &&
          name == o.name
    ...
    def hash
      [is_direct_attach, url, kind, uuid, name].hash
    end
