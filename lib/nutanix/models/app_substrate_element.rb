...
module Nutanix
  # Substrate element definition for Application
  class AppSubstrateElement
    # 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::AppSubstrateElement` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::AppSubstrateElement`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [platform_data, instance_address, instance_power_state, action_list, instance_id, uuid, name, instance_name, state, readiness_probe, editables, message_list, os_type, type, create_spec, variable_list, description].hash
    end
