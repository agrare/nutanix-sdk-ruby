...
module Nutanix
  # Deployment related config for this gateway.
  class Deployment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::Deployment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::Deployment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@ip_prefix_length.nil? && @ip_prefix_length > 32
        invalid_properties.push('invalid value for "ip_prefix_length", must be smaller than or equal to 32.')
      end

      if !@ip_prefix_length.nil? && @ip_prefix_length < 0
        invalid_properties.push('invalid value for "ip_prefix_length", must be greater than or equal to 0.')
      end
      ...
      if !@installed_software_version.nil? && @installed_software_version.to_s.length > 1000
        invalid_properties.push('invalid value for "installed_software_version", the character length must be smaller than or equal to 1000.')
      end
    ...
    def valid?
      ...
      return false if !@ip_prefix_length.nil? && @ip_prefix_length > 32
      return false if !@ip_prefix_length.nil? && @ip_prefix_length < 0
      ...
      return false if !@installed_software_version.nil? && @installed_software_version.to_s.length > 1000
    ...
    # @param [Object] ip_prefix_length Value to be assigned
    def ip_prefix_length=(ip_prefix_length)
      if ip_prefix_length.nil?
        fail ArgumentError, 'ip_prefix_length cannot be nil'
      end

      if ip_prefix_length > 32
        fail ArgumentError, 'invalid value for "ip_prefix_length", must be smaller than or equal to 32.'
      end

      if ip_prefix_length < 0
        fail ArgumentError, 'invalid value for "ip_prefix_length", must be greater than or equal to 0.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] installed_software_version Value to be assigned
    def installed_software_version=(installed_software_version)
      if installed_software_version.nil?
        fail ArgumentError, 'installed_software_version cannot be nil'
      end

      if installed_software_version.to_s.length > 1000
        fail ArgumentError, 'invalid value for "installed_software_version", the character length must be smaller than or equal to 1000.'
      end
    ...
    def hash
      [install_lb_route, vcenter_deployment_details, ip_prefix_length, static_ip, default_gateway_ip, image_source_url, pe_cluster_reference, subnet_reference, installed_software_version, vlan_id].hash
    end
