...
module Nutanix
  # Details of the VM/Volume virtual disk.
  class DiskSpec
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::DiskSpec` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::DiskSpec`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @total_size_bytes.nil?
        invalid_properties.push('invalid value for "total_size_bytes", total_size_bytes cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @total_size_bytes.nil?
