...
module Nutanix
  ...
  class VolumeGroupResourcesInput
    ...
    def self.openapi_types
      {
        ...
        :'attachment_list' => :'Array<AttachmentReferenceInput>',
        :'usage_type' => :'String',
        :'target_secret' => :'String',
        :'is_hidden' => :'Boolean',
        :'disk_list' => :'Array<VolumeDiskResourceInput>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::VolumeGroupResourcesInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::VolumeGroupResourcesInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@iscsi_target_prefix.nil? && @iscsi_target_prefix.to_s.length > 512
        invalid_properties.push('invalid value for "iscsi_target_prefix", the character length must be smaller than or equal to 512.')
      end

      pattern = Regexp.new(/^[\da-zA-Z:\.\-]+$/)
      if !@iscsi_target_prefix.nil? && @iscsi_target_prefix !~ pattern
        invalid_properties.push("invalid value for \"iscsi_target_prefix\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if !@iscsi_target_prefix.nil? && @iscsi_target_prefix.to_s.length > 512
      return false if !@iscsi_target_prefix.nil? && @iscsi_target_prefix !~ Regexp.new(/^[\da-zA-Z:\.\-]+$/)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] iscsi_target_prefix Value to be assigned
    def iscsi_target_prefix=(iscsi_target_prefix)
      if iscsi_target_prefix.nil?
        fail ArgumentError, 'iscsi_target_prefix cannot be nil'
      end

      if iscsi_target_prefix.to_s.length > 512
        fail ArgumentError, 'invalid value for "iscsi_target_prefix", the character length must be smaller than or equal to 512.'
      end

      pattern = Regexp.new(/^[\da-zA-Z:\.\-]+$/)
      if iscsi_target_prefix !~ pattern
        fail ArgumentError, "invalid value for \"iscsi_target_prefix\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [flash_mode, load_balance_vm_attachments, created_by, iscsi_target_prefix, parent_reference, sharing_status, attachment_list, usage_type, target_secret, is_hidden, disk_list].hash
    end
