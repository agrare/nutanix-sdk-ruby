  ...
  class AppResources
    ...
    def self.openapi_types
      {
        ...
        :'credential_list' => :'Array<AppCredentialInput>',
        ...
        :'deployment_list' => :'Array<AppDeploymentInput>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::AppResources` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::AppResources`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [action_list, credential_list, app_blueprint_config_reference, app_blueprint_reference, app_profile_config_reference, client_attrs, dependency_list, variable_list, deployment_list].hash
    end
