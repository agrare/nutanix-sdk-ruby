  ...
  class SupportCaseResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::SupportCaseResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::SupportCaseResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [status, log_collector, contact_details, ncc_checks_pc, creator_name, case_number, nos_version, creation_date, priority, hypervisor_version, case_type, case_id, custom_key_values, owner, serial_number, additional_notification_list, log_collector_pc, cluster_reference, ncc_checks].hash
    end
