...
module Nutanix
  # Section Details. Section is a group of widgets.
  class Section
    attr_accessor :repetition_criteria

    # List of template rows.
    attr_accessor :template_rows
    ...
    def self.openapi_types
      {
        ...
        :'template_rows' => :'Array<TemplateRow>',
        :'description' => :'String',
        :'name' => :'String',
        :'section_id' => :'String'
      }
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::Section` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::Section`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'template_rows')
        if (value = attributes[:'template_rows']).is_a?(Array)
          self.template_rows = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @section_id.nil?
        invalid_properties.push('invalid value for "section_id", section_id cannot be nil.')
      end

      if @section_id.to_s.length > 64
        invalid_properties.push('invalid value for "section_id", the character length must be smaller than or equal to 64.')
      end
    ...
    def valid?
      ...
      return false if @section_id.to_s.length > 64
      true
    end
    ...
    def section_id=(section_id)
      if section_id.nil?
        fail ArgumentError, 'section_id cannot be nil'
      end

      if section_id.to_s.length > 64
        fail ArgumentError, 'invalid value for "section_id", the character length must be smaller than or equal to 64.'
      end
    ...
    def hash
      [repetition_criteria, template_rows, description, name, section_id].hash
    end
