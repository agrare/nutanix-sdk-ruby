...
module Nutanix
  ...
  class BlueprintDeploymentInputUpload
    # 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::BlueprintDeploymentInputUpload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::BlueprintDeploymentInputUpload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [percent_fault_tolerance, fault_domain_scope, description, action_list, package_local_reference_list, published_service_local_reference_list, uuid, name, editables, depends_on_list, max_replicas, type, substrate_local_reference, variable_list, min_replicas, options, num_fault_tolerance, brownfield_instance_list].hash
    end
