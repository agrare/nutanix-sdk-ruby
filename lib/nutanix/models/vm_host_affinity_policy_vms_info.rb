...
module Nutanix
  # VMs information which are part of VM Host Affinity Policy.
  class VmHostAffinityPolicyVmsInfo
    # Compliance status.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::VmHostAffinityPolicyVmsInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::VmHostAffinityPolicyVmsInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@reason.nil? && @reason.to_s.length > 1000
    ...
    def valid?
      ...
      return false if !@reason.nil? && @reason.to_s.length > 1000
      true
    end
    ...
    def hash
      [status, host_reference, cluster_reference, vm_categories, reason, vm_reference].hash
    end
