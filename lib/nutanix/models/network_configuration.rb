  ...
  class NetworkConfiguration
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::NetworkConfiguration` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::NetworkConfiguration`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ip_info_list')
        if (value = attributes[:'ip_info_list']).is_a?(Array)
          self.ip_info_list = value
        end
      end
    end
    ...
    def hash
      [dns_ip_addresses_list, name, mac_address, dhcp_server_ip, default_gateway_address_list, ip_info_list].hash
    end
