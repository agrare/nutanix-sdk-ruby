  ...
  class AppTaskResponseDownload
    ...
    def self.openapi_types
      {
        ...
        :'child_tasks_local_reference_list' => :'Array<AppTaskReferenceUpload>',
        :'name' => :'String',
        :'state' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::AppTaskResponseDownload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::AppTaskResponseDownload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [target_any_local_reference, retries, description, message_list, child_tasks_local_reference_list, name, state, attrs, timeout_secs, type, variable_list, uuid].hash
    end
