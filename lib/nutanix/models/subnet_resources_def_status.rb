...
module Nutanix
  ...
  class SubnetResourcesDefStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::SubnetResourcesDefStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::SubnetResourcesDefStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ip_usage_stats')
        self.ip_usage_stats = attributes[:'ip_usage_stats']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ip_usage_stats == o.ip_usage_stats &&
    ...
    def hash
      [subnet_type, vpc_reference, virtual_switch_uuid, is_external, vswitch_name, network_function_chain_reference, reserved_ip_address_list, availability_zone_reference_list, ip_config, ip_usage_stats, enable_nat, external_connectivity_state, vlan_id, virtual_network_reference].hash
    end
