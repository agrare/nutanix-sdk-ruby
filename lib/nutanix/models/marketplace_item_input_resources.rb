...
module Nutanix
  ...
  class MarketplaceItemInputResources
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::MarketplaceItemInputResources` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::MarketplaceItemInputResources`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @author.to_s.length > 64
        invalid_properties.push('invalid value for "author", the character length must be smaller than or equal to 64.')
      end
    ...
    def valid?
      ...
      return false if @author.to_s.length > 64
    ...
    def author=(author)
      ...
      if author.to_s.length > 64
        fail ArgumentError, 'invalid value for "author", the character length must be smaller than or equal to 64.'
      end
