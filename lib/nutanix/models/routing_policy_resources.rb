  ...
  class RoutingPolicyResources
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::RoutingPolicyResources` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::RoutingPolicyResources`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @priority > 1000
        invalid_properties.push('invalid value for "priority", must be smaller than or equal to 1000.')
      end
    ...
    def priority=(priority)
      ...
      if priority > 1000
        fail ArgumentError, 'invalid value for "priority", must be smaller than or equal to 1000.'
      end
    ...
    def hash
      [is_bidirectional, vpc_reference, destination, priority, source, protocol_parameters, virtual_network_reference, action, protocol_type].hash
    end
