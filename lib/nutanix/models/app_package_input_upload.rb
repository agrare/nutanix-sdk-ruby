  ...
  class AppPackageInputUpload
    ...
    def self.openapi_types
      {
        ...
        :'service_local_reference_list' => :'Array<AppServiceReferenceUpload>',
        :'name' => :'String',
        :'version' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::AppPackageInputUpload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::AppPackageInputUpload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [image_spec, description, action_list, service_local_reference_list, name, version, editables, config_reference, type, options, variable_list, uuid].hash
    end
