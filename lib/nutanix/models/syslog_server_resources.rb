...
module Nutanix
  # Remote Syslog server API resources definition
  class SyslogServerResources
    ...
    def self.openapi_types
      {
        :'module_list' => :'Array<SyslogModule>',
        :'ip_address' => :'String',
        :'port' => :'Integer',
        :'server_name' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nutanix::SyslogServerResources` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nutanix::SyslogServerResources`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'module_list')
        if (value = attributes[:'module_list']).is_a?(Array)
    ...
    def hash
      [module_list, ip_address, port, server_name, network_protocol].hash
    end
